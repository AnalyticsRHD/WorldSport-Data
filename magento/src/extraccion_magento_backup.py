...
def obtener_catalogo_skus(oauth, page_size=200, solo_habilitados=True):
    """
...
if __name__ == "__main__":
    ...
    if extraer_stock:
        # Stock de TODO el catalogo (no solo los SKUs que vendieron en la
        # ventana reciente) -- por eso se trae la lista de SKUs aparte, via
        # obtener_catalogo_skus(), en vez de reusar skus_unicos de ordenes.
        print("\nTrayendo catalogo completo de SKUs...")
        skus_catalogo = obtener_catalogo_skus(oauth)
 
        print(f"\nConsultando stock para {len(skus_catalogo)} SKUs del catalogo...")
        df_stock = importar_stock(oauth, skus_catalogo)
